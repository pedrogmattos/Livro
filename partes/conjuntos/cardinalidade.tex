...
\begin{proof}
	...
	\item Se $\card{X} = \card{Y}$ e $\card{Y} \text{\ \ e\ \ } \card{Y}$, então existem bijeções $C_1: X \to Y$ e $C_2: Y \to Z$. Mas então $C_2 \circ C_1 : X \to Z$ é uma bijeção de $X$ em $Y$ e, portanto, $\card{X} = \card{Z}$.
	\end{enumerate}
\end{proof}
...
\end{theorem}
\begin{proof}
Mostremos a primeira desigualdade. Seja $j \in I$ tal que
	\begin{equation*}
	\card{C_j} := \min_{i \in I}{\card{C_i}}
	\end{equation*}
e $C := C_j$. Para cada $i \in I$, existe função injetiva $f_i: C \to C_i$. Considere a função
	\begin{align*}
