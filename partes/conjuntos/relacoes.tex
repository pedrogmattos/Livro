...
\begin{definition}[Cobertura e subcobertura]
...
\begin{proposition}[Refinamento é pré-ordem]
...
Nas seções seguintes, estudaremos um tipo de cobertura chamado partição. Quando restrita a essas coberturas, a relação será antissimétrica, portanto uma ordem.
...
% Não defini cardinalidade ainda nesse ponto e quero simplificar a demonstração para não usar subcoberturas finitas.
\begin{comment}
\begin{proposition}
...
\subsection{Correfinamento de coberturas}

%Podemos definir uma operação entre duas coberturas que mantém informação de ambas as coberturas, mas de modo que as refine.

\begin{definition}[Correfinamento de coberturas]
...
As definições coincidem se consideramos uma família $(\cober_0, \cober_1)$ de duas coberturas. Nesse caso,
	\begin{equation*}
...
\begin{enumerate}
	...
	\item Para todas coberturas $\cober$ e $\cober'$ de $X$ e todas subcoberturas $\mathcal{S} \subseteq \cober$ e $\mathcal{S'} \subseteq \cober'$, vale
		...
		\end{equation*}
\end{enumerate}
%Se $\mathcal{S}$ $\cober'_0$ são subcoberturas, então $\mathcal{S} \corref \mathcal{S'} \subseteq \cober \corref \cober'$.
\end{proposition}
\begin{proof}
	\begin{enumerate}
...
\begin{exercise}[Propriedades de reticulado]
Seja $X$ um conjunto.
	\begin{enumerate}
	...
	\item (Limitação inferior) Para toda cobertura $\cober \in \Cober(X)$,
		...
		\end{equation*}
	% Essa seguinte não é verdade pois o correfinamento com $\p_1(X)$ pode ter o conjunto vazio $\emptyset$ como elemento.
%	\item Para toda cobertura $\cober \in \Cober(X)$,
%		\begin{equation*}
%		\cober \corref \p_1(X) = \p_1(X).
%		\end{equation*}
	\end{enumerate}
\end{exercise}

%Vale comentar que a última propriedade nem sempre vale quando consideramos coberturas abertas de espaços topológicos, pois a cobertura atômica $\p_1(X)$ (composta dos subconjuntos de cardinalidade $1$) não necessariamente é uma cobertura aberta --- esses conjuntos nem sempre são abertos.
...
%O mesmo vale para qualquer conjunto de coberturas, mesmo que infinito ou até não enumerável, mas esses refinamentos nem sempre são úteis na prática.
...
Basta notarmos que, para todo $C'' \in \cober''$, vale
	...
	\qedhere
	\end{equation*}
%Assim, para todo $C' \in (f_1 \circ f_0)\inv (\cober)$, algum $C \in \cober$ satisfaz $C' = (f_1 \circ f_0)\inv(C)$, o que implica que $C' \in f_0\inv (f_1\inv(\cober))$ e a recíproca é análoga.
%Reciprocamente, seja $C' \in f_0\inv (f_1\inv(\cober))$. Então, para algum $C \in \cober$, vale $C' = f_0\inv(f_1\inv(C)$, logo $C' = (f_1 \circ f_0)\inv(C)$, portanto $C' \in (f_1 \circ f_0)\inv (\cober)$.
\end{proof}

Isso nos permite usar sem ambiguidade, para $\fun{f}{X}{X}$ e uma cobertura $\cober$ de $X$, a notação
	\begin{equation*}
...
\begin{proposition}[Refinamento é ordem]
...
\begin{definition}[Subpartição e superpartição]
...
\subsection{Correfinamento de partições}
...
\begin{definition}[Correfinamento de partições]
