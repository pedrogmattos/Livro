...
%MOSTRAR QUE A DEFINIÇÃO ACIMA E A ABAIXO SÃO EQUIVALENTES. Acredito que basta tirar os pontos $f(x),...,f^N-1(x) de V.$
%\begin{definition}
%Seja $\bm{\Sist}=(\bm X,f)$ um sistema dinâmico topológico. Um ponto \emph{errante} do sistema é um ponto $p \in X$ para o qual existem vizinhança $V \subseteq X$ de $p$ e inteiro positivo $N \in \Z_{\geq 0}$ tais que
%	\begin{equation*}
%	V \cap \bigcup_{t \in \Z_{\geq 0}}f^{N+t}(V) = \emptyset.
%	\end{equation*}
%Um ponto \emph{não-errante} do sistema é um ponto $p \in X$ que não é errante: para toda vizinhança $V \subseteq X$ de $p$ e inteiro positivo $N \in \Z_{\geq 0}$,
%	\begin{equation*}
%	V \cap \bigcup_{t \in \Z_{\geq 0}}f^{N+t}(V) \neq \emptyset.
%	\end{equation*}
%\end{definition}
...
%	\begin{equation*}
%	V \cap \bigcup_{t \in \Z_{\geq 0}}f^t(V) = \emptyset.
%	\end{equation*}
...
%	 = \bigcap_{n \in \N} \Fec{\Orb^+(f^n(p))}.
	\end{equation*}
%	\begin{equation*}
%	\Lim^+_f(p) := \set{q \in X}{\exists (t_n)_{n \in \N} \in (\Z_{\geq 0})^\N,\ f^{t_n}(p) \conv q}.
%	\end{equation*}
...
%Note que
%	\begin{align*}
%	\omega_f(p) &= \set{q \in X}{\exists (t_n)_{n \in \N} \in (\Z_{\geq 0})^\N,\ f^{t_n}(p) \conv q} \\
%		&= \set{\lim_{t_n \conv \infty} (f^{t_n}(p))}{(t_n)_{n \in \N} \in (\Z_{\geq 0})^\N} \\
%		&= \bigcup_{t \in (\Z_{\geq 0})^\N} \{\lim_t (f^t(p)\}.
%	\end{align*}
