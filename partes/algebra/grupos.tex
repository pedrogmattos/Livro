...
%\begin{definition}
%Um \emph{grupo} é um grupíneo $\bm G=(G,\times,\div,1)$ cuja operação binária $\times$ associativa. Um \emph{grupo comutativo} é um grupo cuja operação binária é comutativa.
%\end{definition}
...
Em vista das definições introdutórias do capítulo anterior, um grupo é um grupíneo associativo, ou seja, um monoide $\bm G=(G, \times, \id)$ cujos elementos têm inverso sob $\times$ e um grupo comutativo é um grupo cuja operação binária $\times$ é comutativa.
...
Seja $\bm G$ um grupo.
	...
	\item Para todo $i \in \Z$ e todos $g,g' \in G$ tais que $gg'=g'g$,
		...
		(gg')^i = g^i(g')^i.
		\end{equation*}
	\item Para todos $g_0, \ldots, g_{n-1} \in G$,% $(g_i)_{i \in [n]} \in G^n$,
		\begin{equation*}
		(g_0 \cdots g_{n-1})\inv = g_{n-1}\inv \cdots g_0\inv
		\end{equation*}
%		\begin{equation*}
%		\left( \bigtimes_{i \in [n]} g_i \right)\inv = \bigtimes_{i \in [n]} g_{n-1-i}\inv
%		\end{equation*}
	\end{enumerate}
\end{exercise}

\begin{exercise}[Cancelamento]
Seja $\bm G$ um grupo.
	\begin{enumerate}
...
%	Pela primeira e segunda propriedades de $S$, segue que $\times_S$ é uma operação binária (\ref{prop:fechamento.operacao.binaria}).

	\item (Associatividade) Segue de \ref{prop:propriedade.subgrupideo}.
%	(\ref{G1}) Sejam $s_1,s_2,s_3 \in S$. Então
%		\begin{equation*}
%		(s_1 \times_S s_2) \times_S s_3 = (s_1 \times s_2) \times s_3 = s_1 \times (s_2 \times s_3) = s_1 \times_S (s_2 \times_S s_3).
%		\end{equation*}
...
%	(\ref{G2}) 	como $1$ é identidade de $\bm G$, segue que, para todo $s \in S$,
%		\begin{equation*}
%		1 \times_S s = 1 \times s = s = s \times 1 = s \times_S 1.
%		\end{equation*}
...
%		\begin{equation*}
%		s_1 \times_S s_2 = s_1 \times s_2 = s_2 \times s_1 = s_2 \times_S s_1.
%		\qedhere
%		\end{equation*}
	\end{enumerate}
\end{proof}
...
Sejam $\bm G$ um grupo e $\bm S \leq \bm G$ um subgrupo. Então
	...
	\end{equation*}
%	\begin{equation*}
%	\card{G}=\card{\quo{G}{S}} \times \card{S}.
%	\end{equation*}
\end{proposition}
\begin{proof}
...
%	\begin{align*}
%		\func{D}{G}{\Iso{\Func}(G)}{g}{\trD{g}}
%	\end{align*}
%
%A \emph{translação à esquerda} em $\bm G$ é a função
%	\begin{align*}
%		\func{E}{G}{\Iso{\Func}(G)}{g}{\trE{g}}
%	\end{align*}
