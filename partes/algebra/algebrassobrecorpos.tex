...
O produto $\times$ pode também ser visto como uma ação de $\R^2$ aditivo sobre $\R^2$ aditivo. Cada $x \in \R^2$ pode ser identificado com uma transformação linear $x\colon \R^2 \to \R^2$. Os elementos $x \in \S^1 \subseteq \R^2$ são as rotações: se $x \in \S^1$, então existe $\theta \in \intfa{0}{\tau}$ tal que
	\begin{equation*}
	x = (\cos(\theta),\sin(\theta)) = \cos(\theta) + \sin(\theta) \ii.
	\end{equation*}
Isso ocorre porque, se $x \in \S^1$, então $\nor{x} = 1$, logo $({x_0}^2+{x_1}^2)^\frac{1}{2} = 1$, portanto ${x_0}^2+{x_1}^2 = 1$. Então $\abs{x_0} \leq 1$, ou seja, $-1 \leq x_0 \leq 1$. Definido $\theta' := \cos\inv(x_0) \in \intff{0}{\frac{\tau}{2}}$, temos que ${x_1}^2 = 1-\cos(\theta)^2$, logo $\abs{x_1} = (1-\cos(\theta)^2)^\frac{1}{2}$, então $x_1 = \pm\sin(\theta)$. Tomamos $\theta := \theta'$ se $x_1 > 0$ e $\theta := \tau - \theta'$ se $x_1 < 0$. No caso $x_1 = 0$, tomamos $\theta = 0$ se $x_0=1$ e $\theta = \frac{\tau}{2}$ se $x_0=-1$.

Sendo assim, para todo $\cos(\theta) + \sin(\theta) \ii \in \S^1$, e todo $x \in \R^2$,
	\begin{align*}
	(\cos(\theta) + \ii \sin(\theta)) \times x &= (\cos(\theta)x_0-\sin(\theta)x_1) + \ii(\cos(\theta)x_1+\sin(\theta)x_0).
	\end{align*}

%Definimos $\angle(x) := \theta$.
Os elementos $x \in \R \subseteq \R^2$ são as expansões e contrações. Os elementos de $\R^2$ podem ser decompostos como
%	\begin{equation*}
%	x = \nor{x}(\cos(\angle(x)) + \sin(\angle(x)) \ii).
%	\end{equation*}
	\begin{equation*}
...
Seja $x \in \R^4$.
	\begin{enumerate}
	\item Para todo $x \in \R^4$, $\conju{x} = -\frac{1}{2}(x + \ii x \ii + \jj x \jj + \kk x \kk)$;
...
%		\vec{(xy)} &= \esc{x}\vec{y}+\esc{y}\vec{x}+\vec{x} \times \vec{y}
	...
	\item Para todo $x \in \R^4 \setminus \{0\}$,% existem $\phi_0 \in \intff{0}{\tau \div 2}$ e $\hat{x} \in \vec{\S}^2$ tais que
		\begin{equation*}
...
Segue que $\esc{u}=0$ e $\nor{\vec{u}}=1$ se, e somente se, $u^2=-1$.
	...
	Das definições de $\phi_0$ e $\hat{x}$, segue que $\esc{x} = \nor{x}\cos(\phi_0)$ e $\vec{x} = \nor{x}\sin(\phi_0)\hat{x}$, portanto
		...
		x &= \esc{x} + \vec{x} \\
			...
			&= \nor{x} (\cos(\phi_0) + \sin(\phi_0) \hat{x}).									\qedhere
		\end{align*}
\end{enumerate}
\end{proof}
