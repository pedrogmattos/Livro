...
Sejam $\bm E$ um espaço normado, $A \subseteq E$ aberto e $\fun{f}{A}{\R}$ um campo escalar. Um ponto crítico \emph{degenerado} de $f$ é um ponto crítico $x \in A$ de $f$ tal que $f$ é $2$-diferenciável em $x$ e
	...
	\end{equation*}
não é um isomorfismo de espaços lineares topológicos. %Umponto crítico  \emph{não-degenerado} de $f$ é um ponto crítico $x \in A$ de $f$ tal que $f$ é $2$-diferenciável em $x$ e $x$ não é degenerado, ou seja, $\fun{\D^2 f|_x}{E}{E\dual}$ é isomorfismo de espaços lineares topológicos.
...
\begin{proof}
	...
	\item ($\Rightarrow$) Como $\Gamma_0$ e $\Gamma$ são primitivas de $\gamma$, então, para todo $t \in \Int{I}$
		...
		\end{equation*}
	Assim, a difrencial de $\Gamma - \Gamma_0$ deve ser nula, portanto segue da proposição \ref{prop:diferencial.zero.constante} que, para algum $c \in E$, $\Gamma - \Gamma_0 = c$ em $\Int{I}$. Como $\Gamma_0$ e $\Gamma$ são contínuas, segue que vale $\Gamma - \Gamma_0 = c$ em $I$.
...
% MOTARAR QUE F É CONTÍNUA ANTES DE USAR A PROPOSIÇÃO COROLÁRIA DA CONTRAÇÃO DO PONTO FIXO.
...
Isso mostra que $F$ é contínua, pois é controlada. Além disso, como $\frac{(c\varepsilon)^n}{n!}$ é um termo da série exponencial de $\e^{c\varepsilon}$, deve valer que, para algum $n \in \N \setminus \{0\}$,
	...
	\end{equation*}
o que implica que $F^n$ é uma contração. Segue então da proposição~\ref{prop:ponto.fixo.iterada.contracao} que $F$ tem único ponto fixo $\bar\phi$. Por definição de $F$, esssa trajetória satisfaz, para todo $t \in \Fec{I}$,
	\begin{equation*}
...
Consideremos para a proposição seguinte um espaço normado completo $\bm E$ e uma transformação linear contínua $L \in \toplin(\bm E, \bm E)$. Essa transofrmação é um campo vetorial em $E$. Esse campo vetorial é controlado, pois a continuidade significa que, para todos $x,x' \in E$,
	\begin{equation*}
	\nor{L(x') - L(x)} \leq \nor{L}\nor{x'-x}.
	\end{equation*}
