...
\end{proposition}
\begin{proof}
Como $\pi_i = \pi_i \circ \Id_X$, segue de uma propriedades básica de imagem inversa de produto (\ref{conj:proposition.im.inv.prod}) que
	\begin{equation*}
...
Consideremos uma esfera $d$-dimensional $\S^d$ e o espaço $\S^d \dotplus \S^d$, dado aqui por
	\begin{align*}
	\S^d \dotplus \S^d &= \left(\frac{1}{2}\S^d + \left(\frac{1}{2},0,\ldots,0\right)\right) \cup \left(\frac{1}{2}\S^d - \left(\frac{1}{2},0,\ldots,0\right)\right) \\
		&= \set{x \in \R^{d+1}}{\nor{x-\left(\frac{1}{2},0,\ldots,0\right)}=\frac{1}{2}} \\
		&\qquad \cup \set{x \in \R^{d+1}}{\nor{x+\left(\frac{1}{2},0,\ldots,0\right)}=\frac{1}{2}}.
	\end{align*}
...
Sejam $\bm X$ um espaço topológico e $A,B \subseteq X$. Definimos as seguintes relações entre $A$ e $B$:
	...
	\end{equation*}
	\item (\emph{Separação por função contínua}) Existe $f \in \Cont(X,[0,1])$ tal que
	\begin{equation*}
	f(A)=\{0\} \text{\ \ e\ \ } f(B)=\{1\}.
	\end{equation*}
	\item (\emph{Separação precisa por função contínua}) Existe $f \in \Cont(X,[0,1])$ tal que
	\begin{equation*}
...
\end{proposition}
\begin{proof}
Seja $(C_I)_{i \in I}$ uma cobertura aberta de $f(\bm X_0)$. A família
	\begin{equation*}
	(f\inv(C_i))_{i \in I}
	\end{equation*}
é  uma cobertura de $X_1$, pois
	\begin{equation*}
...
Consideremos, assim, a função
	...
	\item Análogo ao item anterior, mas considerando as parametrizações $\phi$, $\fun{\phi'}{[0,1]}{[0,1]}$
	\begin{equation*}
	\phi(s)
		=
			\begin{cases}
