...
%	\begin{align*}
%	\func{\dist{\var}{\var}_\phi}{M \times M}{\intfa{0}{\infty}}{(p,p')}{\phi(\dist{p}{p'})}
%	\end{align*}
é uma métrica em $M$.
\end{proposition}
\begin{proof}
%Note que a monotonicidade crescente estrita de $\phi$ implica que, para todo $c \in \intfa{0}{\infty}$, se $\phi(c) = 0$ então $c=0$.
	\begin{enumerate}
	\item (Separação) Sejam $p,p' \in M$. %Se $\phi(\dist{p}{p'}) = 0$, da mediabilidade segue que $\dist{p}{p'} = 0$, e da separação de $\dist{\var}{\var}$ segue que $p=p'$.
%	Se $p=p'$, então da separação de $\dist{\var}{\var}$ segue que $\dist{p}{p'} = 0$, portanto
%		\[
...
%Seja $\fun{\phi}{\intfa{0}{\infty}}{\intfa{0}{\infty}}$ uma função côncava. Então, para todos $c,c',c'' \in \intfa{0}{\infty}$ tais que $c<c'<c''$,
	\[
...
\end{lemma}
\begin{proof}
%Sejam $c,c',c'' \in \intfa{0}{\infty}$ tais que $c<c'<c''$
...
É possível mostrar que se uma função $\fun{\phi}{\intfa{0}{\infty}}{\intfa{0}{\infty}}$ torna $\dist{\var}{\var}_\phi$ uma métrica para toda métrica $\dist{\var}{\var}$, então ela deve ser mediável e subaditiva. No entanto, existe uma tal função $\phi$ que é contínua, mas não é crescente.
...
Seja $(M,\dist{\var}{\var})$ um espaço métrico.
	...
	\item A função% a \emph{métrica limitada induzida por $\dist{\var}{\var}$}
		\begin{align*}
...
Além de criar novas métricas a partir de uma que se tem usando uma função $\fun{\phi}{\intfa{0}{\infty}}{\intfa{0}{\infty}}$, podemos usar várias métricas diferentes.
...
%	\begin{align*}
%	\func{\dist{\var}{\var}_{\S^d}}{\S^d}{\S^d}{(x,x')}{\cos\inv(\inte{x}{x'})}.
%	\end{align*}
%\end{definition}
...
A recíproca nem sempre é verdade. Nos espaços $\R^d$, $d \in \N$, vale que um conjunto é compacto se, e somente se, é fechado e limitado. Esse resultado é conhecido como Teorema de Heine-Borel. No entanto, isso não vale em qualquer espaço métrico%
	\footnote{
...
Nesta seção, expandimos o conceito de uma função controlada usando um parâmetro.
...
\subsection{Equivalência de métricas}
...
%	\item As estruturas uniformes induzidas em $M$ por $\dist{\var}{\var}$ e $\dist{\var}{\var}'$ são iguais.
