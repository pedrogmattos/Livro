...
Note que valem as seguintes propriedades.
...
Primeiro mostremos que, para toda carta $(A,\x) \in \atlas$, a carta induzida $(\Tg A, \bar x) \in \Tg \atlas$ é um carta de $\Tg V$ modelada em $\bm E \times \bm E$.
	\begin{itemize}
%		\item ($\fun{\Tg \x}{\Tg A}{E \times E}$ é injetiva)
...
%		\begin{itemize}
%		\item ($\subseteq$) Seja $(y, u) \in \Tg \x(\Tg A)$. Então existe $(p, v) \in \Tg A$ tal que $\Tg \x(p,v) = (y,u)$. Então $p \in A$, logo $\x(p) \in \x(A)$, e $v \in \Tg V|_p$, logo $\D \x|_p(v) \in E$, portanto $\Tg \x(p,v) = (\x(p), \D \x|_p(v)) \in \x(A) \times E$.
%		
%		\item ($\supseteq$) Seja $(y, u) \in \x(A) \times E$. Como $y \in \x(A)$, existe $p \in A$ tal que $\x(p) = y$. Tomando $[u]_{\x}|_p \in \Tg V|_p$, segue que $(p, [u]_{\x}|_p) \in \Tg A$ e $\D \x|_p([u]_{\x}|_p) = u \in E$, portanto $(y,u) = \Tg \x(p,[u]_{\x}|_p) \in \x(\Tg A)$.
%%		$(y,v) = (\x(p), \D \x|_p([v]_{\x}|_p)) = \Tg \x(p, [v]_{\x}|_p) \in \Tg \x(\Tg A)$.
%		\end{itemize}			
...
%	\begin{itemize*}
%	\item [($\subseteq$)] Seja $(p,v) \in \Tg A \cap \Tg A'$. Então $p \in A$ e $p \in A'$, logo $p \in A \cap A'$, o que implica $\x(p) \in \x(A \cap A')$; além disso, $\D \x|_p(v) \in E$, logo $(\x(p), \D \x|_p(v)) \in \x(A \cap A') \times E$ e $\Tg \x(p,v) = (\x(p), \D \x|_p(v))$.
%	
...
%	\end{itemize*}
	...
	% É verdade que a trnsição de coordenadas será $\Cont^{1+r}$-diferenciável, mas essa última parte precisa ser explicada melhor, pois a função $\D (\x' \circ \x\inv)|_y(u)$ na segunda entrada da expressão para a transição de coordenadas não é função só de $y$, mas de $u$ também; é uma função de $(y,u)$ com domínio $\x(A \cap A') \times E$. Essa função é de fato difereniável, pois sua diferencial parcial na primeira entrada é
